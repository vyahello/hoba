...
- `apps/api/uv.lock` — generated by `uv run` invocations during this cleanup pass's quality-gate runs. Never tracked in git history; not in `.gitignore`. Modern uv guidance recommends committing the lockfile for reproducible builds; the repo's 2026-05-27 github-push prep (commit `2e8b2d0`) didn't address it either way. Decision needed: (a) commit `apps/api/uv.lock` for reproducibility, OR (b) add it to `.gitignore` to formalize the exclude. Not auto-fixed — this is a real policy call the cleanup pass shouldn't make unilaterally.
