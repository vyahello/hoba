...
- [ ] stage:D — chaos — Slice 6 shipped the "safe 5" events. Two spec §5.4 events were deferred: **Double-spin** (two real spins, both count — needs a 2-result settle payload / mini-series timing) and **Phantom segment** (an ephemeral non-DB 13th segment — needs a wire shape + client render path for a segment with no id). Add as a follow-up slice if playtests want more variety.
