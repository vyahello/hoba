...
- [ ] stage:D — chaos — still open from spec §5.4: **Double-spin** (two fully-counted results — needs a 2-result settle payload) and **Phantom segment** (ephemeral non-DB 13th segment — needs a wire shape + client render for a segment with no id). Add as a follow-up slice if playtests want more variety. (Done in the rework: multi_spin, slow_burn ×3, reverse, visible swap, nudge_fwd/back with shake.)
- [ ] stage:D — chaos — **on-device animation tuning** for the Chaos choreography constants in `RoomPage.tsx`: `MULTI_SPIN_STEP_MS`/`MULTI_SPIN_FINAL_MS` (multi_spin pacing), `NUDGE_SHAKE_MS`/`NUDGE_CREEP_MS` (nudge feel), and `SLOW_BURN_MULTIPLIER` in `modes/chaos.py` (currently ×3). Written blind (no device); revisit after the owner sees them on a real phone.
