...
- [ ] stage:D — chaos — **nudge ±1 event** (owner-requested, deferred from the Slice 6 rework). After the wheel settles on segment X it creeps one sector forward/back to X±1, changing the result — the "it changed!" drama. Needs a two-phase wheel animation: server records the nudged result + the pre-nudge angle in `mode_effects` (e.g. `nudge_from_angle`); RoomPage animates to the pre-nudge stop, pauses, then to the nudged angle (a local second `SpinResult` fed to `<Wheel>`), then reveals. Best-of-N tally already counts the recorded (nudged) `result_segment_id`, so no tally change. Deferred because it needs on-device animation tuning. Also still open from spec §5.4: **Double-spin** (two real spins, both count) and **Phantom segment** (ephemeral non-DB 13th segment).
