...
### The fifteen chaos events (uniform, never the same one twice running)
...
| `jammed` 🔒 | a **fully-locked wheel**: it strains and shudders but **never moves** — it stays exactly where it is and the result is the segment already under the pointer | service keeps `final_angle_deg = starting_angle_deg` and sets the result to `segment_under_pointer(starting_angle_deg)`; client strains forward + snaps back to the start angle ×4 (shudder + strained-grind SFX + haptic each) and settles there — it never breaks free |
