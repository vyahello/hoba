...
| `jammed` 🔒 | a **"stuck wheel"**: it strains forward and **snaps back** (staying put) a few times, then **breaks free** and spins to the result | engine names it; client lurches forward + snaps back to the start angle ×3 (strained clunk SFX + haptic each), then a normal spin to the server result |
