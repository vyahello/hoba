...
- **Bot turn pacing** — the solo bot now "reacts" like a real player: a *randomized* beat (`_bot_turn_delay_seconds`, secrets-jittered **2.8–5.0 s**) between the turn passing to it and its spin, so it never feels robotically instant, and the human can read the previous result first. Replaced the fixed `BOT_TURN_DELAY_SECONDS`. Covers every bot hand-off (spin-settle, dare-resolve/approve, rejoin-resume).
