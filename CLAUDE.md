...
>
> **Chaos mode (spec §5.4) — committed 2026-05-31, reworked same day, pending owner deploy + real-device verify.** Fourth `GameModeEngine`, "full chaos": **an event fires on every spin** — one of four (speed_run ×0.5, slow_burn ×1.5, reverse CCW, swap two segments; jackpot removed). Host picks **attempts N∈{1,3,5,7}** → best-of-N (most-frequent wins, reuses Classic machinery). Spin policy `turn_based` default; settings offers host_only/turn_based only (no "anyone"). Announcement card leads with skewed `Хоба!`. Server-authoritative (`modes/chaos.py`), folded into `spin:started.mode_effects`, no DB change. **Deferred (`docs/TODO.md`): nudge ±1** (owner-requested, needs two-phase wheel anim), Double-spin, Phantom-segment. Detail in [`docs/changelog.md`](docs/changelog.md) Slice 6 + [`docs/game-modes.md`](docs/game-modes.md).
>
