...
## Current phase
> **Stage C CLOSED (2026-05-28, owner-override GO on Phase 7+).** Production has been live on `hobagame.duckdns.org` since 2026-05-27. Stage B closed (11 items inc. 2 verification-pass fixes); shared VPS, host nginx as TLS terminator, Hoba containers on `127.0.0.1:8800/5800/6800`; webapp container is a multi-stage build serving a static bundle via in-container nginx (pulled Stage G TODO forward). BotFather menu button + Direct Link Mini App URL switched to prod. Test totals at code freeze: 113 backend / 53 frontend, 91 % coverage. **No third-party soft-launch sessions ran**; owner instead verified the soft-launch path end-to-end on iPhone 14 + iPhone X across both locales (2026-05-28) and reports no defects — see `docs/validation-notes.md` § Owner-side verification. The roadmap's "two third-party play sessions" criterion is being **overridden by owner judgement**, not satisfied. iPhone X spin-lag fix (commit `33a976b`) verified smooth on A11 in the same pass. **Cleanup pass (2026-05-28)** paid down Stage A/B/C debt across 17 commits (12 cleanup + 5 triage) without changing project state — see `docs/cleanup-2026-05-28-findings.md`. **Stage D unblocked.** Next stage per `docs/roadmap.md` is Stage D — Phase 7: Classic + Elimination + Punishment + Chaos modes.
