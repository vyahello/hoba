...
## Current phase
> **Stage E — Rigged Mode 🎭 (spec §5.5) feature-complete (both slices), committed 2026-05-31. Pending owner real-device verify, then `STAGE E COMPLETE` close-out.**
> **Rigged:** host secretly weights segments (0–100 in `Segment.weight`, consumed by `compute_spin`) via a hidden ~1.5 s **long-press on the hub** → `RigEditorSheet`; room flips to `game_mode="rigged"` but **`build_room_state` redacts** it to "classic" + uniform weights for non-host viewers (indistinguishable from Classic) until the host reveals. **Reveal**: host-only "Reveal the rig 🎭" → `POST /rooms/{code}/reveal` flips `rigged_revealed` (un-redacts for all), stamps 🎭 into the title, broadcasts `rigged:revealed` → clients refetch + play a full-screen reveal (skewed Хоба! + bar-chart + "rigged for N spins" + confetti). Host-only endpoints don't broadcast pre-reveal. Alembic 0013 (`rigged_revealed`) + 0014 (`rigged_spin_count`). CI guards weighted-RNG fairness (±2%). Detail in [`docs/changelog.md`](docs/changelog.md) (Stage E) + [`docs/game-modes.md`](docs/game-modes.md). Verify on device, then `STAGE E COMPLETE`. After Stage E: **Stage F** (library / saved wheels / templates / public + trending + moderation).
>
