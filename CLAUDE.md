...
>
> **Chaos mode (spec §5.4) — committed 2026-05-31, reworked twice same day, pending owner deploy + real-device verify.** Fourth `GameModeEngine`, "full chaos": **an event fires on every spin** — one of six (multi_spin 2–5 fast spins, slow_burn ×3, reverse CCW, swap two segments [shown crossing], nudge_fwd/nudge_back = settle→shake→creep ±1). Host picks **attempts N∈{1,3,5,7}** → best-of-N (reuses Classic machinery). Spin policy `turn_based` default; settings offers host_only/turn_based only. Client choreography is an async cancellable sequence (`phaseSpin` override + `wheelRef`, container shake via `useAnimate`); tunable constants flagged for on-device tuning. Server-authoritative (`modes/chaos.py`), folded into `spin:started.mode_effects`, no DB change. **Deferred (`docs/TODO.md`): Double-spin, Phantom-segment.** Detail in [`docs/changelog.md`](docs/changelog.md) Slice 6 + [`docs/game-modes.md`](docs/game-modes.md).
>
