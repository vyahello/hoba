...
## Current phase
> **Stage C open — NO-GO on Phase 7+ recorded 2026-05-28.** Production has been live on `hobagame.duckdns.org` since 2026-05-27 (Stage B closed: 11 items inc. 2 verification-pass fixes; shared VPS, host nginx as TLS terminator, Hoba containers on `127.0.0.1:8800/5800/6800`; webapp container rewritten as a multi-stage build serving a static bundle via in-container nginx, which pulled the Stage G TODO forward). BotFather menu button + Direct Link Mini App URL switched to prod. Test totals at code freeze: 113 backend / 53 frontend, 91 % coverage. **However, no soft-launch sessions were logged**, so the Stage C exit criterion (written go/no-go grounded in real-user signal) could not be met on evidence — the verdict in `docs/validation-notes.md` is **NO-GO**, rationale = insufficient validation. **Stage C stays open**, not closed. No Stage D scoping, no Phase 7+ work until at least two real-device sessions (iOS + Android, EN + UK) are logged in `docs/validation-notes.md` and the synthesis re-run. `docs/soft-launch-invite.md` still has the friend invite copy. **No code work in Stage C** — only telemetry + feedback.
