...
## Current phase
> **Stage E in progress — Rigged Mode 🎭 (spec §5.5). Slice 1 (secret rigging mechanic) committed 2026-05-31; next is Slice 2 (the reveal moment).**
> **Rigged Slice 1:** host secretly weights segments (0–100 in `Segment.weight`, already consumed by `compute_spin`) via a hidden ~1.5 s **long-press on the hub** → `RigEditorSheet`; the room flips to `game_mode="rigged"` but **per-viewer redaction in `build_room_state`** shows non-host viewers a plain "classic" room with uniform weights (so it's indistinguishable from Classic). Host-only `PATCH /rooms/{code}/rig` (`services/rigged.py`, no broadcast). New `rooms.rigged_revealed` col (Alembic 0013), always False until Slice 2. Host sees a 🎭 badge; CI guards weighted-RNG fairness (±2%). Detail in [`docs/changelog.md`](docs/changelog.md) (Stage E Slice 1) + [`docs/game-modes.md`](docs/game-modes.md). **Slice 2 = "Reveal the rig"** (flip `rigged_revealed` → un-redact + bar-chart + 🎭 title + rigged-spin-count). Gets its own brainstorming arc on `go`.
>
...
>
> **Carry-overs into Stage E (`docs/TODO.md`):** Chaos animation-constant tuning (`buildRoamHops` pace, `SLOW_BURN_TURNS`, `NUDGE_*`) is feedback-driven and still open; deferred Chaos events **Double-spin** + **Phantom-segment**. Stage E = Rigged Mode (host secretly weights segments, guests see "Classic", long-press reveal finale) — `compute_spin` already supports server-side weights.
>
