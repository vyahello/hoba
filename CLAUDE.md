...
## Current phase
> **🎉 STAGE G COMPLETE (closed 2026-06-01) — ALL STAGES A→G DONE. The roadmap is fully delivered; the codebase implements the entire `docs/spec.md` (all five modes, library + discovery, host moderation, hardening, launch content). Remaining work is OWNER-ONLY launch ops (see `docs/TODO.md` `stage:G — owner-only`): BotFather final config (Privacy URL → `/privacy`), record the README walkthrough GIF, fresh-VPS <30 min smoke, real-device 360px audit + device-verify the Stage G slices. No more `go on stage X` — the next work is feature requests, polish carry-overs, or owner launch tasks.**
> **Stage G delivered Phase 11 + 12 in three slices:** (1) **Host moderation toolkit** — kick (`kicked_at` + `POST /rooms/{code}/kick` + `room:kicked` broadcast; re-join blocked), lock enforcement, **anonymous mode** (deterministic adjective+animal nicknames in `anon.py`, localized to the viewer's language, redacted in `build_room_state`), mid-room mode change (resets round state), close-with-confirm (`room:closed` broadcast); all surfaced in a reworked host `RoomSettingsSheet` (lock/anon toggles, Players+kick, change mode, close) + native Telegram confirm. (2) **Hardening** — `sanitize.py` (NFKC + strip control/zero-width + collapse) applied at every text write boundary, `tasks/cleanup.py` room-cleanup task (CLI + cron in `docs/deployment.md`), API fuzz suite asserting no 5xx; replay + all §14 rate limits audited (already present). (3) **Launch content+ops** — in-app `/privacy` + `/terms` (EN+UK `legal` namespace) linked from Settings, `scripts/deploy.sh`, README 60-sec walkthrough + deploy section. Full close-out in [`docs/changelog.md`](docs/changelog.md) (STAGE G COMPLETE).
>
> Stages A–F (all closed): **A** MVP hardening, **B** pre-launch hardening, **C** deploy chain (prod live), **D** four `GameModeEngine` modes (Classic/Elimination/Punishment/Chaos), **E** Rigged Mode 🎭 (all five modes), **F** library + templates + public/trending/moderation (Alembic up to 0016). Detail in [`docs/changelog.md`](docs/changelog.md) + [`docs/game-modes.md`](docs/game-modes.md).
>
