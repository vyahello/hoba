...
>
> **Chaos mode (spec §5.4) — committed 2026-05-31, pending owner deploy + real-device verify.** Shipped as the fourth `GameModeEngine`: ~25% of spins roll one of five events (speed_run ×0.5, slow_burn ×1.5, reverse CCW, swap two segments, jackpot flourish), announced on a ~1.5 s card before the wheel fires; the rest are Classic. Server-authoritative roll (`modes/chaos.py`), folded into `spin:started.mode_effects` (no separate `chaos:event`), no DB change. Double-spin + Phantom-segment from §5.4 are deferred (`docs/TODO.md`). Detail in [`docs/changelog.md`](docs/changelog.md) Slice 6 + [`docs/game-modes.md`](docs/game-modes.md).
>
